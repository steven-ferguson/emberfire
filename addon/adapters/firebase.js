...
export default DS.Adapter.extend(Waitable, {
   ...
   * @return {String}
   */
  generateIdForRecord(store, modelName) {
    const path = this.buildPath(modelName);
    const newRecordRef = this.ref.child(path).push();
    return newRecordRef.key;
  },

  findRecord(store, typeClass, id) {
    const path = this.buildPath(typeClass.modelName, id);
    const ref = this.ref.child(path);

    return this.fetchAndListen(store, typeClass.modelName, ref)
      .then(snapshot => {
        if (!snapshot.exists()) {
          const error = new Error(`no record was found at ${snapshot.ref.toString()}`);
          error.recordId = id;
          throw error;
        }
  ...
  createRecord(store, typeClass, snapshot) {
    const path = this.buildPath(typeClass.modelName, snapshot.id);
    const data = this.serialize(snapshot, { path });
  ...
  updateRecord(store, typeClass, snapshot) {
    const path = this.buildPath(typeClass.modelName, snapshot.id);
    const data = this.serialize(snapshot, { path });
    return this.ref.update(data).then(() => {
      return Promise.resolve();
    });
  },

  fetchAndListen(store, modelName, ref) {
    let callCount = 0;
    return new Promise((resolve, reject) => {
      const listener = function (snapshot) {
        ...
        if (callCount === 1) {
          ...
          store.push(store.normalize(modelName, snapshot));
        }
      };

      this.listeners[ref.path.toString()] = listener;
  ...
  findAll(store, typeClass) {
    const path = this.buildPath(typeClass.modelName);
  ...
  query(store, typeClass, query, recordArray) {
    const path = this.buildPath(typeClass.modelName);
    const ref = this.ref.child(path);
    const fbQuery = this.applyQueryToRef(ref, query);

    return fbQuery.once('value');
